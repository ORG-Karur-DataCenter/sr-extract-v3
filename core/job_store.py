...
class JobStore:
    """Thread-safe-ish wrapper around SQLite for the extraction pipeline."""
    ...
    def mark_done(self, job_id: str, result: dict, model_used: str):
        ...
        with self._conn() as c:
            ...
            # Bump study progress
            row = c.execute("SELECT study_id FROM jobs WHERE id=?", (job_id,)).fetchone()
            if row:
                c.execute(
    ...
    # ── Query helpers ────────────────────────────────────────────────
    def get_study_chunks(self, study_id: str) -> list[sqlite3.Row]:
        with self._conn() as c:
            return c.execute(
    ...
    def get_completed_studies(self) -> list[sqlite3.Row]:
